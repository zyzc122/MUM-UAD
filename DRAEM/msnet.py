...
class MSNet(nn.Module):
    def __init__(self, num_branch=1, alpha=10):
        ...
        self.res_2 = nn.Sequential(ConvBlock(512, 256, 3, padding=1, stride=1, bn=True, act=nn.ReLU()),
                                   ConvBlock(256, 64, 3, padding=1, stride=1, bn=True, act=nn.ReLU()),
                                   ConvBlock(64, 1, 3, padding=1, stride=1, bn=True, act=nn.ReLU()))
        # self.msb1 = Merge_Split_Block(80, 128, 16, 24, stride=2)
        # self.msb2 = Merge_Split_Block(120, 128, 24, 32, stride=2)
        # self.msb3 = Merge_Split_Block(160, 256, 32, 40,stride=2)
        # self.msb4 = Merge_Split_Block(200, 512, 40, 48,stride=2)
        # self.msb1 = MergeBlock(80, 128, 120, stride=2)
        # self.msb2 = MergeBlock(120, 128, 160,stride=2)
        # self.msb3 = MergeBlock(160, 256, 200,stride=2)
        # self.msb4 = MergeBlock(200, 512, 240,stride=1)
    ...
    def forward(self, flabels, fea_list):
        ...
        fea_ = self.attn1(flabels_, fea_list[1], pos_embed)
        fea_ = self.attn2(fea_, fea_list[2])
        fea_ = self.attn3(fea_, fea_list[4])
        out = self.res_2(fea_)
