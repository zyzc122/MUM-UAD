...
def test(obj_names, mvtec_path, checkpoint_path):
    ...
    model.load_state_dict(torch.load(os.path.join(checkpoint_path+"all"+".pckl"), map_location='cuda:0'))
    model.cuda()
    model.eval()

    model_seg = DiscriminativeSubNetwork(in_channels=6, out_channels=2)
    model_seg.load_state_dict(torch.load(os.path.join(checkpoint_path+"all"+"_seg.pckl"), map_location='cuda:0'))
    model_seg.cuda()
    model_seg.eval()
