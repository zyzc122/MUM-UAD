...
def MVTec_Path_Split(train_ratio, root_image,
    ...
    for dename in defect_name:
        ...
        # 随机打乱 并 按比例划分
        path = f'{root_image}/{category}/test/{dename}/'
        files = glob(f'{path}*.JPG')
        files.extend(glob(f'{path}*.png'))
        files.extend(glob(f'{path}*.bmp'))
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            for method in self.methods:
                # self.label_refine_files.extend()
                flabel_path = os.path.join(self.root_flabels, method, category, dename,
                                           os.path.split(image_file)[1].split('.')[0]) + '.png'
