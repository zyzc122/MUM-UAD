...
def test(obj_names, mvtec_path, checkpoint_path,saveimages):
    obj_ap_pixel_list = []
    ...
    allDataset = ConcatDataset(test_list)
    dataloader = DataLoader(allDataset, batch_size=1,
    ...
    model.load_state_dict(torch.load(os.path.join(checkpoint_path), map_location='cuda:0'))
    model.cuda()
    model.eval()
    # dataset = MVTecDRAEMTestDataset(mvtec_path + obj_name + "/test/", resize_shape=[img_dim, img_dim])
    ...
    total_pixel_scores = np.zeros((img_dim * img_dim * len(allDataset)))
    total_gt_pixel_scores = np.zeros((img_dim * img_dim * len(allDataset)))
    mask_cnt = 0
...
if __name__=="__main__":
    ...
    with torch.cuda.device(args.gpu_id):
        test(obj_list,args.data_path, args.checkpoint_path,args.saveimages)
