...
class MVTecDRAEMTestDataset_Saliency(Dataset):
    def __init__(self, root_dir,  resize_shape=None):
        self.root_dir = root_dir
        self.images = sorted(root_dir)
...
class MVTecDRAEMTrainDataset_Saliency(Dataset):

    def __init__(self, root_dir, anomaly_source_path, methods, root_flabels, resize_shape=None):
        """
        Args:
        ...
        self.image_paths = sorted(root_dir)
    ...
    def __getitem__(self, idx):
        ...
        image = self.transform_image(self.image_paths[idx], None)
        preds_list = []
        dename = self.image_paths[idx].split("/")[-2]
        category = self.image_paths[idx].split('/')[-4]
        file_name = self.image_paths[idx].split('/')[-1].split('.')[0]
        for method in self.methods:
            ...
            pred = np.array(Image.open(flabel_path).convert('L'))
