...
parser.add_argument("--config", default="/home/scu-its-gpu-001/UniAD_Gradient/experiments/vis_query/config.yaml")
...
def main():
    ...
    for k in model_query.keys():
        if "reconstruction.output_proj." in k:
            k_new = k.replace("reconstruction.output_proj.", "")
    ...
    for idx_layer in range(config.vis_query.num_decoder_layers):
        k_query = f"reconstruction.transformer.decoder.layers.{idx_layer}.learned_embed.weight"
