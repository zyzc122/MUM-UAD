...
class ResNet(nn.Module):
    def __init__(
        ...
        self.gradient1 = nn.Sequential(
        nn.Upsample(scale_factor=2, mode="bilinear"),
        nn.Conv2d(self.inplanes, self.inplanes, kernel_size=1, stride=1, bias=False),
        nn.ReLU(inplace=True),nn.BatchNorm2d(self.inplanes),

        )
        self.gradient2 = nn.Sequential(
            nn.Conv2d(self.inplanes, self.inplanes, kernel_size=1, stride=1, bias=False),
            nn.ReLU(inplace=True), nn.BatchNorm2d(self.inplanes ),
        )
        self.gradient3 = nn.Sequential(
            nn.Conv2d(self.inplanes , 1, kernel_size=1, stride=1, bias=False),
        ...
        self.mask3 = nn.Sequential(
            nn.Conv2d(self.inplanes*2 , self.inplanes, kernel_size=1, stride=1, bias=False),
            nn.ReLU(inplace=True),nn.BatchNorm2d(self.inplanes),
            nn.Conv2d(self.inplanes , 1, kernel_size=1, stride=1, bias=False),
            nn.ReLU(inplace=True), nn.BatchNorm2d(1),
            # nn.Sigmoid()
        ...
        self.mask = nn.Sequential(
            nn.Conv2d(self.inplanes, self.inplanes, kernel_size=(3, 3), padding=1),
            nn.ReLU(inplace=True),nn.BatchNorm2d(self.inplanes),
            nn.Conv2d(self.inplanes, self.inplanes, kernel_size=(3, 3), padding=1),
            nn.ReLU(inplace=True),nn.BatchNorm2d(self.inplanes),
            nn.Upsample(scale_factor=2, mode="bilinear"),
            nn.Conv2d(self.inplanes, 1, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(1),
            # nn.Sigmoid()
        )
...
model = ReverseNet(block_type="basic", instrides=[16], inplanes=[3])

output = model(torch.randn(1, 3, 224, 224))  # 测试2个样本
