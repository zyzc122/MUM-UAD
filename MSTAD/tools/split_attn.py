...
class Split_Attn(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pos_embed = build_position_embedding(
            pos_embed_type, feature_size, 128
        )
    ...
    def forward(self, flabel_feature, img_feature_list):
        ...
        atten_feature = self.transformer(
            flabel_tokens, img_feature_list, pos_embed,
        )  # (H x W) x B x C
...
class Transformer(nn.Module):
    ...
    def forward(self, flabel_feature, img_feature_list, pos_embed):
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = nn.MultiheadAttention(128, nhead, dropout=dropout)
        self.multihead_attn = nn.MultiheadAttention(hidden_dim, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(hidden_dim, 68)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(hidden_dim, 60)
        self.linear3 = nn.Linear(128, 48)
        self.linear4 = nn.Linear(204, 256)
    ...
    def forward_post(
        ...
        feature_low = self.linear1(img_feature)
        flabel_low = self.linear2(out)
        xm = torch.cat([feature_low, flabel_low], 2)
        xm = self.self_attn(
            query=self.with_pos_embed(xm, pos),
            key=self.with_pos_embed(xm, pos),
            value=xm,
            attn_mask=tgt_mask,
            key_padding_mask=tgt_key_padding_mask,
        )[0]
        xm = xm + self.dropout1(xm)
        xm = self.norm1(xm)
        xm = self.linear3(xm)

        xs = torch.split(flabel_low, 20, 2)
        xs_c = []
        for x_i in xs:
            xs_c.append(torch.cat([x_i, xm], 2))
        tgt = torch.cat(xs_c, 2)
        ...
        #
        # tgt = self.linear2(self.dropout(self.activation(self.linear1(tgt2))))
