...
if __name__ == "__main__":
    ...
    # 计算Memery footprint
    # ============================================================================
    # optimizer_mstad = get_optimizer(model.parameters(), config.trainer.optimizer)
    # loss_MSTAD = torch.nn.MSELoss()
    ...
    #
    # torch.cuda.reset_peak_memory_stats()
    # optimizer_mstad.zero_grad()
    # outputs = model(input)
    # loss1 = loss_MSTAD(outputs['feature_align'], outputs['feature_rec'])
    # loss1.backward()
    # optimizer_mstad.step()
    # peak_mem = torch.cuda.max_memory_allocated() / 1e9
    # print(f"Train MSTAD Memery:{peak_mem}GB")
    ...
    print(f"Test MSTAD Memery:{peak_mem}GB")
