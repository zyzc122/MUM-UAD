...
class MFCN(nn.Module):
    ...
    def forward(self, input):
        ...
        # features_mask = input["features_mask"]
        ...
        for i in range(4):
            ...
            # feature_resize_mask = upsample(features_mask[i])
            feature_list.append(feature_resize)
            # feature_list_mask.append(feature_resize_mask)

        feature_align = torch.cat(feature_list, dim=1)
        # feature_align_mask = torch.cat(feature_list_mask, dim=1)

        return {"feature_align": feature_align,
                # "feature_align_mask": feature_align_mask,
