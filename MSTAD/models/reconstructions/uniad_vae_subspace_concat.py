...
class UniAD(nn.Module):
    ...
    def forward(self, input):
        ...
        output_decoder, enc_list, decoder_list = self.transformer(
            feature_tokens1,
            pos_embed1
        )  # (H x W) x B x C
...
class Transformer(nn.Module):
    ...
    def forward(self, src1,pos_embed1):
        ...
        return output_decoder1, enc_list, decoder_list
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        # gradient自注意力
        # tgt2 = self.self_gradient_attn(
        #     query=self.with_pos_embed(gradient, pos_embed2),
        #     key=self.with_pos_embed(gradient, pos_embed2),
        #     value=gradient,
        # )[0]
        # gradient = gradient + self.dropout0(tgt2)
        # gradient = self.norm0(gradient)

        # tgt2 = self.gradient_head_attn(
        #     # query=self.with_pos_embed(tgt, pos),
        #     # key=self.with_pos_embed(gradient, pos),
        #     # value=gradient,
        #     query=self.with_pos_embed(gradient, pos_embed2),
        ...
        # )[0]
        # # tgt2 = self.activation(self.adaptor_down_2(tgt2))
        # # tgt2 = self.activation(self.adaptor_up_2(tgt2))
        # src = src + self.dropout3(tgt2)
        # src = self.norm3(src)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        # tgt2 = tgt_common2 + self.dropout3(tgt2)
