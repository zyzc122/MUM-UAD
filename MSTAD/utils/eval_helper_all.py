...
mvtec_list = ['cable', 'zipper', 'capsule', 'transistor', 'bottle', 'hazelnut',
               'metal_nut',  'toothbrush', 'pill', 'screw',
               'leather', 'carpet',  'tile', 'grid', 'wood']
btad_list = ['01', '02',  '03', ]
...
def performances(fileinfos, preds, masks, config):
    ...
    if config.get("auc", None):
        for metric in config.auc:
            ...
            ret_metrics["{}_{}_{}_auc".format("mvtec", "mean", evalname)] = objectmean_auc
            ret_metrics["{}_{}_{}_auc".format("btad", "mean", evalname)] = texturemean_auc
...
def log_metrics(ret_metrics, config):
    ...
    clsnames.remove('mvtec_mean')
    clsnames.remove('btad_mean')

    clsnames.append('mvtec_mean')
    clsnames.append('btad_mean')
