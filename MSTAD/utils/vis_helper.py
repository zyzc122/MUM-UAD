...
def visualize_pic(fileinfos, preds, cfg_vis, cfg_reader):
    ...
    vis_dir = '/home/smart-solution-server-003/anomaly_PycharmProjects/Saliency_MSTAD_from3090/Saliency_MSTAD/tools/'+vis_dir
    ...
    for i, fileinfo in enumerate(fileinfos):
        ...
        pred = preds[i]
        threshold = numpy.percentile(preds, 97)

        pred = torch.from_numpy(pred)
        binary_pred = (pred >= threshold).float()
        # pred = transforms.Nor malize(mean=[-0.485, -0.456, -0.406],
        #                              std=[np.float(1.0 / 0.229), np.float(1.0 / 0.224), np.float(1.0 / 0.225)])(
        #     pred)
        binary_pred = binary_pred.numpy()
        # pred = pred[::-1,:,:]
        # pred = np.transpose(pred, (1, 2, 0))
        binary_pred = cv2.resize(binary_pred, (w, h))
        binary_pred = (binary_pred * 255).astype(np.float)
        # pred = np.transpose(pred, (2, 0, 1))
        save_path = os.path.join(save_dir, filename)

        # scoremap = np.vstack([image, pred])
        cv2.imwrite(save_path, binary_pred)
...
def visualize_compound(fileinfos, preds, masks, cfg_vis, cfg_reader):
    ...
    for i, fileinfo in enumerate(fileinfos):
        ...
        _, defename = os.path.split(filedir)
        save_dir = os.path.join(vis_dir, clsname, defename)
        save_dir = '/home/smart-solution-server-003/anomaly_PycharmProjects/Saliency_MSTAD_from3090/Saliency_MSTAD/tools/{}'.format(save_dir)
        os.makedirs(save_dir, exist_ok=True)
